...
|```<div type=cast=[characters]>```|The tag specifies the dramatic characters present in the scene. It is usually necessary to specify the dramatic characters who are present in the "extra" scene.|```<div``` type="extra_scene"```cast="Лиза, София, Фамусов">```
